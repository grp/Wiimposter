...
def summary(report, item):
	tmd = wii.TMD("%08x%08x/tmd" % (item.titleid >> 32, item.titleid & 0xFFFFFFFF)) #not tmp/ because we are already in tmp
...
def detailed(report, item):
	tmd = wii.TMD("tmp/%08x%08x/tmd" % (item.titleid >> 32, item.titleid & 0xFFFFFFFF))
	tik = wii.Ticket("tmp/%08x%08x/tik" % (item.titleid >> 32, item.titleid & 0xFFFFFFFF))
	...
	shutil.copy("tmp/%08x%08x/certs" % (item.titleid >> 32, item.titleid & 0xFFFFFFFF), "nand/sys/certs.sys")
	shutil.copy("tmp/%08x%08x/certs" % (item.titleid >> 32, item.titleid & 0xFFFFFFFF), "encnand/sys/certs.sys")
		...
		for elem in old:
			...
			for title in soap:
				if(elem.titleid != title.titleid):
					cookie_jar -= 1
				elif(elem.version != title.version or elem.size != title.size):
